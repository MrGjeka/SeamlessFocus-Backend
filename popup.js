...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function init() {
        try {
            const storedData = await getStoredData(['spotifyAccessToken', 'spotifyTokenExpiration', 'selectedPlaylistId', 'currentPlaylistId']);
            console.log('Checking for stored access token');
            if (storedData.spotifyAccessToken) {
                // Check if token has expired
                if (Date.now() > storedData.spotifyTokenExpiration) {
